  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        ...
        const shuffledPosts = [...fetchedPosts].slice(0,20).sort(() => 0.5 - Math.random())
        setRandomPosts(shuffledPosts.slice(0, shuffledPosts.length))
      } catch (error) {
      ...
      <div className="max-w-7xl mx-auto px-6 py-2 space-y-4">
        ...
        <div className='flex flex-row justify-between items-center'>
          ...
          <div className='flex flex-row gap-2 items-center'>
            ...
            {/* Search Icon */}
            <button
              className="p-1 text-gray-600 hover:text-yellow-500 hover:bg-yellow-50 cursor-pointer border rounded-lg transition-all duration-200"
              title="Ara"
        ...
        <div>
          <h2 className="text-3xl font-bold text-gray-900 mb-2 mt-8">Son Eklenenler</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {randomPosts.length === 0 ? (
              <div className="bg-gray-50 rounded-lg border border-gray-200 p-6 text-center">
                <p className="text-gray-500">Ürünler bulunamadı.</p>
              </div>
            ) : (
